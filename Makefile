spiral_generative.mp4 spiral_hybrid.mp4 spiral_discriminative.mp4: spiral.py
	...
	ffmpeg -y \
		...
		-f image2 \
		-s 1920x1080 \
		-i plots/spiral_generative_ssl_%d.png \
		-vcodec libx264 \
		-vf "pad=ceil(iw/2)*2:ceil(ih/2)*2" \
		-crf 25 \
		-pix_fmt yuv420p \
		videos/spiral_generative_ssl.mp4
	ffmpeg -y \
		-r 2 \
		-f image2 \
		-s 1920x1080 \
		-i plots/spiral_hybrid_ssl_%d.png \
		-vcodec libx264 \
		-vf "pad=ceil(iw/2)*2:ceil(ih/2)*2" \
		-crf 25 \
		-pix_fmt yuv420p \
		videos/spiral_hybrid_ssl.mp4
	ffmpeg -y \
		-r 2 \
		-f image2 \
		-s 1920x1080 \
		-i plots/spiral_discriminative_ssl_%d.png \
		-vcodec libx264 \
		-vf "pad=ceil(iw/2)*2:ceil(ih/2)*2" \
		-crf 25 \
		-pix_fmt yuv420p \
		videos/spiral_discriminative_ssl.mp4
